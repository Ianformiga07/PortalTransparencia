...
Const adTypeText = 2
Const xfsCompleted    = &H0 '0  Formulбrio foi processado com sucesso.
Const xfsNotPost      = &H1 '1  Mйtodo de envio nгo й POST.
Const xfsZeroLength   = &H2 '2  Conteъdo de tamanho zero (nгo hб conteъdo no formulбrio de origem)
Const xfsInProgress   = &H3 '3  Formulбrio estб no meio do processo.
Const xfsNone         = &H5 '5  Estado inicial do formulбrio.
Const xfsError        = &HA '10  Se houve algum erro.
Const xfsNoBoundary   = &HB '11  Boundary dos arquivos em formulбrio multipart/form-data nгo foi especificado.
Const xfsUnknownType  = &HC '12  Formulбrio desconhecido (Content-type necessita ser application/x-www-form-urlencoded ou multipart/form-data)
...
Const MaxLicensedLimit = &H77359400 ' Tamanho mбximo do upload para qualquer situaзгo = 2 GB (limite do IIS).
Class ASPForm
 ...
 End Property
  ...
  if NewLimit > MaxLicensedLimit Then
   Err.Raise 1, "Upload grande", "Seu upload ultrapassou o limite mбximo suportado por este servidor."
 ...
 Private m_Items
 Private m_State
 Private m_SizeLimit 'Define o tamanho limite do formulбrio.
...
Class cFormFields
 ...
 Public Default Property Get Item(Key)
  If vartype(Key) = vbInteger or vartype(Key) = vbLong then
   if Key<1 or Key>m_Count Then Err.raise "Item nгo encontrado"
 ...
 End Sub
  ...
  Do While (PosOpenBoundary > 0 And PosCloseBoundary > 0 And Not isLastBoundary)
   ...
   If Not isLastBoundary Then 'Final do boundary. Segue para prуximo arquivo.
...
Class cProgressFile
 ...
 End Property
  Public Sub Done 'Remove arquivo temporбrio quando alcanзa sucesso no upload.
...
Class cFormField
 ...
 End Property
  ...
  SaveAs fullFileName
 Else
  Err.raise "O campo de texto " & Name & " nгo tem um nome de arquivo."
  End If
 End Sub
